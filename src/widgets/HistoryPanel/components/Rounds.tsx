...
const Rounds = ({ best }: bestProps) => {
  const [count, setCount] = useState({ winCount: 0, totalProfit: 0, loseCount: 0, loseProfit: 0, winProfit: 0, percentWin: 0, persentlose: 0 })
        ...
        <Row>
          <Column>
            ...
            <div>
              ...
              <Value i={i} fontSize='13px'>{i === 0 ? `${count.percentWin} %` : i === 1 ? `${count.persentlose} %` : 'Total'}</Value>
            </div>
          </Column>
          <div>
            <Round textAlign='right' i={i}>${i === 0 ? count.winProfit : i === 1 ? count.loseProfit : count.totalProfit}</Round>
            <Value i={i} fontSize='13px' textAlign='right'>$ 0.56</Value>
          </div>
        </Row>
      )}
    </Wrap>
