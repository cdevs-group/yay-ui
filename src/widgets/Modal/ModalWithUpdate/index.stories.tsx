...
import useModalWithUpdate from "./useModalWithUpdate";

export default {
  title: "Widgets/ModalWithUpdate",
  component: Modal,
  argTypes: {},
};

const CustomModal: React.FC<Props> = ({ title, onDismiss, ...props }) => (
...
export const Default: React.FC = () => {
  ...
  const [onPresent1] = useModalWithUpdate(<CustomModal title="Modal 1" />);
  const [onPresent2] = useModalWithUpdate(<CustomModal title="Modal 2" />);
...
export const DisableOverlayClick: React.FC = () => {
  const [onPresent1] = useModalWithUpdate(<CustomModal title="Modal 1" />, false);
...
export const WithBackButton: React.FC = () => {
  const BackButtonModal: React.FC<Props> = ({ title, onDismiss }) => {
    const handleOnBack = () => {
      return 1;
    };
  ...
  const [onPresent1] = useModalWithUpdate(<BackButtonModal title="Modal with no X" />, false);

  return <Button onClick={onPresent1}>Only Back Button</Button>;
};

export const WithCustomHeader: React.FC = () => {
  const CustomHeaderModal: React.FC<Props> = ({ title, onDismiss }) => {
    return (
  ...
  const [onPresent1] = useModalWithUpdate(<CustomHeaderModal title="Modal with custom header" />);
...
export const ReactingToOusideChanges: React.FC = () => {
  ...
  const ReactiveModal: React.FC<Props & { count: number }> = ({ title, count, onDismiss }) => {
    return (
  ...
  const [onPresent1] = useModalWithUpdate(
  ...
  const [onPresent2] = useModalWithUpdate(
